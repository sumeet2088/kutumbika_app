...
class _VehicleFormScreenState extends State<VehicleFormScreen> {
  ...
  Future<void> _loadDocs() async {
    try {
      ...
      setState(() => _documents = (data['documents'] as List?) ?? []);
    } catch (e) {
    ...
    try {
      ...
      if (_file != null) {
        ...
        for (final cat in cats) {
          ...
          if ('${map['document_name']}'.toLowerCase().contains('vehicle')) {
        ...
        categoryRef ??= cats.isNotEmpty
            ...
            : null;
        if (categoryRef == null) {
          throw Exception('Create a Vehicle document category first');
        }
      ...
      await ApiService.instance.createVehicle(
        familyRef: familyRef,
        make: _make.text.trim(),
        model: _model.text.trim(),
        registrationNumber: _reg.text.trim(),
        year: int.tryParse(_year.text.trim()),
        rcDocumentRef: documentRef,
      );
